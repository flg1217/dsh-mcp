...
const MCP_SUPPORTED_PROTOCOL_VERSIONS = [MCP_PROTOCOL_VERSION];
/**
 ...
 */
function isDispatchStopped(error) {
...
/**
 * 命令执行工具(与 dsh-tools-bridge 的 EXECUTION_TOOLS 同族)。
 *
 * 它们的 schema 里 `description` 是**唯一直觉缺失的必填项**——命令正文一眼可猜,
 * UI 说明文字却猜不出来。
 */
const COMMAND_TOOLS = new Set(['pwsh', 'bash']);
/**
 * 补全 `description` 兜底(仅命令执行工具)。
 *
 * 现场实测(2026-09-19,AGY 每次首轮):模型经 MCP 壳 `call_mcp_tool` 调用时
 * 拿不到内层参数 schema(其 thinking 明说"为避免探索不查 schema、直接猜参数"),
 * 传 `{command}` 而漏掉仅用于 UI 的必填 `description` → dsh 校验硬拒,模型拿到
 * 错误后才补参数重试——每次首轮白跑一次(用户报告:"基本上每次第一轮都会出现
 * 同样的失误")。此处按命令正文派生一个描述,保证首次调用即成功。
 * 命令本身缺失时不补:让真实的校验错误去教模型。
 * @param name - dsh 工具名。
 * @param input - 模型传来的参数。
 * @returns 补全后的参数(无需补全时原样返回)。
 */
function withUiDescriptionFallback(name, input) {
    ...
    const derived = firstLine.length > 60 ? `${firstLine.slice(0, 60)}…` : firstLine;
    return { ...input, description: derived };
}
/**
 ...
 * @returns 注销函数。
 */
export function registerMcpLoopDispatcher(sessionId, dispatch) {
    loopDispatchers.set(sessionId, dispatch);
    return () => {
        if (loopDispatchers.get(sessionId) === dispatch)
            loopDispatchers.delete(sessionId);
    };
}
/**
 * 端点状态。
...
const state = (globalThis[ENDPOINT_STATE_KEY] ??= {});
/** 取该会话的存活 agent 面(拿不到返回 undefined;服务缺失不抛)。 */
function liveAgentFace(ctx, sessionId) {
    try {
 ...
 */
function deliverAfterClientGone(ctx, sessionId, toolName, text, reason = 'client-gone') {
...
function bridgeLog(line) {
    ...
    try {
        ...
        appendFileSync(join(homedir(), '.dsh', 'dsh-mcp', 'mcp-bridge.log'), `${new Date().toISOString()} ${line}\n`);
    }
    catch { /* 诊断不影响主流程 */ }
}
/**
 ...
 */
export function dshMcpEndpointUrl(sessionId) {
    if (state.endpoint === undefined || state.resolvePort === undefined)
        return undefined;
 ...
 * @returns 释放函数。
 */
export function registerDshMcpServer(ctx) {
    ...
    const fiber = injectable.inject(['webServer'], (injected) => {
        ...
        const myEndpoint = { key: (state.stableKey ??= randomBytes(18).toString('hex')) };
        ...
        const disposeRoute = web.register({
            ...
            handler: (req, res) => {
                void handleMcpRequest(ctx, req, res).catch((error) => {
...
function keyMatches(candidate) {
    if (state.endpoint === undefined || candidate === null)
        return false;
...
async function handleMcpRequest(ctx, req, res) {
    ...
    if (!loopback)
        return fail(403, 'loopback only');
    const url = new URL(req.url ?? '/', 'http://localhost');
    if (!keyMatches(url.searchParams.get('key')))
        return fail(403, 'bad key');
    ...
    const attachmentsOf = () => ctx.get('attachments');
    /**
     * 工具结果 → MCP `content` 数组。
     *
     ...
     * ——纯文本响应与历史行为逐字一致。
     */
    const contentOf = async (result, text) => {
    ...
    switch (msg.method) {
        ...
        case 'tools/call': {
            ...
            const input = withUiDescriptionFallback(name, args !== null && typeof args === 'object' && !Array.isArray(args)
                ? args
                : {});
            ...
            if (dispatcher !== undefined) {
                try {
                    ...
                    if (isDispatchStopped(error)) {
                        ...
                        // 重复副作用的关键。补投进会话。
                        if (clientGoneAt !== undefined)
                            deliverAfterClientGone(ctx, sessionId, name, advice);
