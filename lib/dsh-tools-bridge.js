 ...
 */
export function isBridgeEligible(name) {
    if (name.length === 0)
        return false;
    if (BRIDGE_EXCLUDED.has(name))
        return false;
    if (platformExcluded(name))
        return false;
...
export function listDshBridgeTools(ctx, parent) {
    ...
    try {
        ...
        console.error('[dsh-mcp] listDshBridgeTools: tools.schemas() 抛错,本次返回空工具表:'
            + (error instanceof Error ? error.message : String(error)));
        return [];
    }
    const specs = [];
    for (const schema of schemas) {
        if (typeof schema?.name !== 'string' || !isBridgeEligible(schema.name))
            continue;
        const inputSchema = (schema.parameters !== null && typeof schema.parameters === 'object'
            ? schema.parameters
            : { type: 'object', properties: {} });
        const base = `${callShapeHint(schema.name, inputSchema)} ${bridgeDescription(schema.name, typeof schema.description === 'string' ? schema.description : '')}`;
        specs.push({
 ...
 */
export function listDshMcpTools(ctx, parent) {
    const tools = ctx.get?.('tools');
    let schemas;
    try {
        schemas = tools?.schemas?.(parent) ?? [];
        ...
        console.error('[dsh-mcp] listDshMcpTools: tools.schemas() 抛错,本次返回空工具表:'
            + (error instanceof Error ? error.message : String(error)));
        return [];
    }
    const specs = [];
 ...
 */
export async function blocksToMcpContent(attachments, content) {
